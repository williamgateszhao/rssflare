...
export const sspaiParser: SiteParser = {
    ...
    async parseList(
        ...
        try {
            ...
            throw new Error(
                `Failed to parse SSPAI list API response as JSON: ${(e as Error).message}`
            );
        }
        ...
        const items: ListItem[] = rawItems.map((item: any) => {
            ...
            const detailApiUrl = item.slug
                ? `https://sspai.com/api/v1/member/article/single/info/get?slug=${encodeURIComponent(item.slug)}&view=second&support_webp=true`
    ...
    async parseDetail(
        ...
        try {
            ...
            throw new Error(
                `Failed to parse SSPAI detail API response as JSON: ${(e as Error).message}`
            );
        }
        ...
        if (Array.isArray(articleData.body_extends)) {
            content += articleData.body_extends
                .map(
                    (section: any) => `<h2>${section.title}</h2>${section.body ?? ""}`
                )
                .join("");
        }
        ...
        const canonicalUrl = postId
            ? `https://sspai.com/post/${postId}`
            : item.url;

        const author =
            articleData.author?.nickname || item.author || undefined;
